...
function getCandidate(address add) public view returns (Candidate[] memory) {
   Candidate[] memory candidateList = new Candidate[](1); //initialize an empty array of length = `nextCandidateId - 1`
    for(uint i = 0; i < nextCandidateId; i++){
        if(candidateDetails[i+1].candidateAddress == add){
        candidateList[0] = candidateDetails[i + 1];
        }
    }
    return candidateList;
}
...
function getVoter(address add) public view returns (Voter[] memory) {
   Voter[] memory voterList = new Voter[](1);
    for(uint i = 0; i < nextVoterId; i++){
        if(voterDetails[i+1].voterAddress == add){
        voterList[0] = voterDetails[i + 1];
        }
    }
    return voterList;
}
